...
import {
  LayoutDashboard, UserCheck, CalendarCog, LogOut, Sun, Moon, Menu
...
const userName = user?.first_name || user?.email?.split("@")[0] || "Receptionist";
...
const navLinks = [
  ...
  { to: "/reception/check-in", label: "Check-In Desk", icon: UserCheck },
  { to: "/reception/slots", label: "Generate Slots", icon: CalendarCog },
];
  ...
  <div class="flex h-screen bg-slate-50 dark:bg-slate-950">
    ...
    >
      <div class="flex h-16 items-center gap-3 border-b border-slate-200 px-5 dark:border-slate-800">
        <div class="flex h-9 w-9 items-center justify-center rounded-xl bg-amber-600 text-sm font-bold text-white">CQ</div>
        <div>
          <p class="text-sm font-bold text-slate-900 dark:text-slate-100">ClinicQueue</p>
          <p class="text-xs text-slate-500 dark:text-slate-400">Reception Desk</p>
        </div>
      </div>
      ...
      <nav class="flex-1 space-y-1 overflow-y-auto px-3 py-4">
        <RouterLink
          ...
          :class="[
            ...
            isActive(link.to) ? 'bg-amber-600 text-white shadow-sm' : 'text-slate-600 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800'
          ]"
        >
    ...
    <div class="flex flex-1 flex-col overflow-hidden">
      <header class="flex h-16 items-center justify-between border-b border-slate-200 bg-white px-4 dark:border-slate-800 dark:bg-slate-950 lg:px-6">
        ...
        <div class="flex items-center gap-2">
          ...
          <div class="flex h-9 w-9 items-center justify-center rounded-full bg-amber-600 text-xs font-bold text-white">{{ userName.charAt(0).toUpperCase() }}</div>
        </div>
      </header>
