...
const doctors = computed(() => {
  if (doctorsRaw.value.length) {
    return doctorsRaw.value.map(normalizeDoctor);
  }

  const doctorUsers = users.value.filter((user) => {
    const groups = user.groups || user.roles || [];
    const role = user.role || user.user_role || "";
    return (
      groups.includes("Doctors") ||
  ...
  return doctorUsers.map(normalizeDoctor);
});
  ...
  <div class="min-h-screen p-6">
    ...
    <div class="mx-auto flex max-w-7xl flex-col gap-6">
      <PageHeader
        ...
        subtitle="Manage weekly schedules, exceptions, and appointment slots."
